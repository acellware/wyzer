...
export class ReportsService {
   ...
   */
  async requestPdf(id: string, organisationId: string): Promise<{ jobId: string | undefined }> {
    const report = await this.findOne(id, organisationId);

    if (!this.storageService.isConfigured()) {
      throw new ForbiddenException('PDF export requires R2 storage to be configured');
    }
   ...
   */
  async createShareToken(id: string, organisationId: string): Promise<{ shareUrl: string; expiresAt: Date }> {
  ...
  async findByShareToken(token: string) {
    ...
    if (!report) throw new NotFoundException('Share link is invalid or has expired');
    return report;
  }
}
